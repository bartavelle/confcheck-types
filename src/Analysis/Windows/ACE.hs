...
data RSID
  = DomainRelative !Word64
  | RSID !SID
  deriving (Eq, Ord, Generic)

data ACL a
  = ACL
      { _aclFlags :: S.Set ACLFlag,
        _aclACEs :: [ACE]
      }
  deriving (Show, Eq)
...
instance Textual (ACL a) where
  textual =
    ACL <$> fmap S.fromList (many textual)
...
instance Textual ACEType where
  textual =
    (SDDL_OBJECT_ACCESS_ALLOWED <$ P.try (P.string "OA"))
...
instance Textual ACEFlag where
  textual =
    (SDDL_CONTAINER_INHERIT <$ P.try (P.string "CI"))
...
instance Textual ACEAccessRight where
  textual =
    (SDDL_GENERIC_ALL <$ P.try (P.string "GA"))
...
instance Printable ACE where
  print (ACE t f r oid ioid sid ra) =
    print t
      <> ";"
      <> foldMap print f
      <> ";"
...
instance Textual ACE where
  textual = do
    ...
    void (P.char ';')
    r <-
      (P.text "0x" *> fmap Left (nonNegative Hexadecimal))
